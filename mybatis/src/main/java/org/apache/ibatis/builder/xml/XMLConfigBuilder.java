...
public class XMLConfigBuilder extends BaseBuilder {
  ...
  private void parseConfiguration(XNode root) {
    try {
      ...
      //解析databaseIdProvider，并将其设置到Configuration中
  ...
  private Properties settingsAsProperties(XNode context) {
    ...
    for (Object key : props.keySet()) {
      //检测Configuration中是否存在相关setter属性，不存在则抛出异常
   ...
   * 别名配置解析，使用别名可方便访问类
   *
   * 方式1：自动扫描的方式配置起来比较简单，唯一缺点就是MyBatis会将某个包下所有符合要求的类的别名都解析出来，并形成映射关系。
   * 如果你不想让某些类被扫描，这个好像做不到，没发现MyBatis提供了相关的排除机制。不过这并不是什么大问题，
   * 最多是多解析并缓存了一些别名到类型的映射，在时间和空间上产生了一些额外的消耗而已。
   * <package name="xyz.coolblog.model1"/>
   *
   * 方式2：通过手工的方式精确配置某些类型的别名。不过这种方式比较繁琐，特别是配置项比较多时。
   *     <typeAlias alias="article" type="xyz.coolblog.model.Article" />
   *     <typeAlias type="xyz.coolblog.model.Author" />
   *
   *
   * 若用户为明确配置alias属性，MyBatis会使用类名的小写形式作为别名。比如，全限定类名xyz.coolblog.model.Author的别名为author。
    ...
    if (parent != null) {
      for (XNode child : parent.getChildren()) {
        if ("package".equals(child.getName())) {
          ...
          //获取resource/url/class 等属性，这三个属性只能同时存在一个
